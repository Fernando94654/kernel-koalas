...
export function NetworkGraph({ nodes, links, height = 360 }: {
  ...
  useEffect(() => {
    ...
    const sim = d3.forceSimulation<Node>(N)
      .force("link", d3.forceLink<Node, d3.SimulationLinkDatum<Node>>(L as unknown as d3.SimulationLinkDatum<Node>[])
    ...
    const link = svg.append("g").attr("stroke", "#cbd5e1")
      ...
      .attr("stroke-width", (d) => 1 + (d.weight / maxW) * 5).attr("stroke-opacity", 0.6);

    const node = svg.append("g").selectAll<SVGGElement, Node>("g").data(N).join("g")
    ...
    node.append("circle")
      ...
      .attr("fill", "#e3001b").attr("fill-opacity", 0.85)
      .attr("stroke", "#fff").attr("stroke-width", 1.5);
    node.append("title").text((d) => `${d.id} (peso ${d.peso})`);
    node.append("text").text((d) => truncate(d.id, 14))
      .attr("x", 8).attr("y", 3).attr("font-size", 8).attr("fill", "#374151");
  ...
  if (!nodes.length) return <p className="text-sm text-muted">Sin datos de grafo.</p>;
